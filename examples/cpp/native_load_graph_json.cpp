...
int main() {
    ...
    auto graph = aexrt::load_graph_json(device, "examples\\add_relu.aexrt.json");

    std::vector<float> a = {-1.0f, 2.0f, -3.0f, 4.0f, -5.0f, 6.0f};
    std::vector<float> b = {3.0f, -4.0f, 5.0f, -6.0f, 7.0f, -8.0f};
    ...
    std::cout << "AEXRT C++ loaded ABI graph max diff: " << max_diff
